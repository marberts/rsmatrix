...
Adding the square of the holding period to the model for the variance, with or without an intercept, is common variation for the interval weights. These weights can all be extended to account for the same house selling more than twice.
...
The GRS index at a point in time is simply the geometric mean of imputed price relatives. This makes it easy to calculate the percent-change contribution of each property at a point in time.

```{r, fig.width=6, fig.height=4}
impute_back <- with(
  subset(prices, period == "2011-01-01" & city == 5),
  price / (price_prev / grs[paste(city, period_prev, sep = ".")])
)

impute_forward <- with(
  subset(prices, period_prev == "2011-01-01" & city == 5),
  price_prev / (price / grs[paste(city, period, sep = ".")])
)

geometric_contributions(c(impute_back, impute_forward)) |>
  density() |>
  plot(main = "Percent-change sales pair contributions", xlab = "", ylab = "")
```